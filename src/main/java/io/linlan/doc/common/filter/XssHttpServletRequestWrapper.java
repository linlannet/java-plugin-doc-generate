...
import io.linlan.doc.common.exception.XssException;
...
public class XssHttpServletRequestWrapper extends HttpServletRequestWrapper {
    ...
    public String[] getParameterValues(String parameter) {
        ...
        for (int i = 0; i < count; i++) {
            if (io.linlan.doc.common.util.ValidateUtil.isContainsForbiddenCharacter(values[i])) {
                throw new XssException("Contains illegal characters[From getParameterValues method]:" + values[i]);
            }
            encodedValues[i] = io.linlan.doc.common.util.StringUtil.cleanXSS(values[i]);
    ...
    public String getParameter(String parameter) {
        ...
        if (io.linlan.doc.common.util.ValidateUtil.isContainsForbiddenCharacter(value)) {
            throw new XssException("Contains illegal characters[From getParameter method]：" + value);
        }
        if (value == null) {
            return null;
        }
        return io.linlan.doc.common.util.StringUtil.cleanXSS(value);
    }
...
//        }
    ...
    public String inputHandlers(ServletInputStream servletInputStream) {
        ...
        if (io.linlan.doc.common.util.ValidateUtil.isContainsForbiddenCharacter(sb.toString())) {
            throw new XssException("Contains illegal characters[From getInputStream method]：" + sb.toString());
        }
        return io.linlan.doc.common.util.StringUtil.cleanXSS(sb.toString());
    }
