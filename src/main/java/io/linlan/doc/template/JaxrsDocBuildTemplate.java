...
import io.linlan.doc.handler.JaxrsHeaderHandler;
import io.linlan.doc.handler.JaxrsPathHandler;
...
import io.linlan.doc.model.request.JaxrsPathMapping;
...
import static io.linlan.doc.constants.DocTags.IGNORE_REQUEST_BODY_ADVICE;
...
public class JaxrsDocBuildTemplate implements IDocBuildTemplate<ApiDoc> {
    ...
    private ApiRequestExample buildReqJson(DocJavaMethod javaMethod, ApiMethodDoc apiMethodDoc, String methodType,
        ...
        if (Methods.POST.getValue()
            ...
            body = io.linlan.doc.common.util.UrlUtil.urlJoin(DocGlobalConstants.EMPTY, DocUtil.formDataToMap(formDataList))
            ...
            path = DocUtil.formatAndRemove(path, pathParamsMap);
            url = io.linlan.doc.common.util.UrlUtil.urlJoin(path, pathParamsMap);
