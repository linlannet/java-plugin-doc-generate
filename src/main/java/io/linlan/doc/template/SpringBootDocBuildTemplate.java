 ...
 * under the License.
 */
package io.linlan.doc.template;

import com.thoughtworks.qdox.model.*;
import com.thoughtworks.qdox.model.expression.AnnotationValue;
import io.linlan.doc.builder.ProjectDocConfigBuilder;
import io.linlan.doc.common.util.ValidateUtil;
import io.linlan.doc.constants.*;
import io.linlan.doc.handler.SpringMVCRequestHeaderHandler;
import io.linlan.doc.handler.SpringMVCRequestMappingHandler;
import io.linlan.doc.helper.FormDataBuildHelper;
import io.linlan.doc.helper.JsonBuildHelper;
import io.linlan.doc.helper.ParamsBuildHelper;
import io.linlan.doc.model.*;
import io.linlan.doc.model.request.ApiRequestExample;
...
public class SpringBootDocBuildTemplate implements IDocBuildTemplate<ApiDoc> {
    ...
    public List<ApiDoc> getApiData(ProjectDocConfigBuilder projectBuilder) {
        ...
        for (JavaClass cls : classes) {
            ...
            order++;
            if (io.linlan.doc.common.util.ValidateUtil.isNonnegativeInteger(strOrder)) {
                setCustomOrder = true;
    ...
    private List<ApiDoc> handleTagsApiDoc(List<ApiDoc> apiDocList) {
        if (io.linlan.doc.common.util.CollectionUtil.isEmpty(apiDocList)) {
        ...
        apiDocList.forEach(doc -> {
            ...
            for (String tag : tags) {
                ...
                copyMap.computeIfPresent(tag, (k, v) -> {
                    List<ApiMethodDoc> list = io.linlan.doc.common.util.CollectionUtil.isEmpty(v.getList()) ? new ArrayList<>() : v.getList();
    ...
    private List<ApiMethodDoc> buildControllerMethod(final JavaClass cls, ApiConfig apiConfig,
        ...
        for (JavaAnnotation annotation : classAnnotations) {
            ...
            if (DocAnnotationConstants.REQUEST_MAPPING.equals(annotationName) ||
                ...
                baseUrl = StringUtilsExt.removeQuotes(DocUtil.getRequestMappingUrl(annotation));
            }
            ...
            if (DocGlobalConstants.FEIGN_CLIENT.equals(annotationName) || DocGlobalConstants.FEIGN_CLIENT_FULLY.equals(annotationName)) {
                baseUrl = StringUtilsExt.removeQuotes(DocUtil.getPathUrl(annotation, DocAnnotationConstants.PATH_PROP));
            }
        }
        ...
        for (DocJavaMethod docJavaMethod : docJavaMethods) {
            ...
            if (Objects.nonNull(pageTag)) {
                ...
                apiMethodDoc.setPage(io.linlan.doc.common.util.UrlUtil.simplifyUrl(pageUrl));
            }
    ...
    private boolean filterPath(RequestMapping requestMapping, ApiReqParam apiReqHeader) {
        if (StringUtils.isEmpty(apiReqHeader.getPathPatterns())
                && StringUtils.isEmpty(apiReqHeader.getExcludePathPatterns())) {
            return true;
        }
    ...
    private ApiRequestExample buildReqJson(DocJavaMethod javaMethod, ApiMethodDoc apiMethodDoc, String methodType,
        ...
        for (JavaAnnotation annotation : methodAnnotations) {
            ...
            if (annotationName.contains("Mapping")) {
                ...
                String params = StringUtilsExt.removeQuotes(paramsObjects.toString());
        ...
        if (parameterList.size() < 1) {
            ...
            String url = io.linlan.doc.common.util.UrlUtil.urlJoin(path, queryParamsMap);
            url = StringUtilsExt.removeQuotes(url);
            url = apiMethodDoc.getServerUrl() + "/" + url;
            url = io.linlan.doc.common.util.UrlUtil.simplifyUrl(url);
        ...
        for (JavaParameter parameter : parameterList) {
            ...
            for (JavaAnnotation annotation : annotations) {
                ...
                if (null != annotationDefaultVal) {
                    mockValue = StringUtilsExt.removeQuotes(annotationDefaultVal.toString());
                }
            ...
            if (JavaClassValidateUtil.isFile(gicTypeName)) {
                ...
                formData.setDescription(comment);
                formData.setType("text");
                formData.setValue(io.linlan.doc.common.util.RandomUtil.randomValueByType(gicName));
                formDataList.add(formData);
        ...
        if (Methods.POST.getValue()
            ...
            body = io.linlan.doc.common.util.UrlUtil.urlJoin(DocGlobalConstants.EMPTY, formDataToMap)
            ...
            path = DocUtil.formatAndRemove(path, pathParamsMap);
            url = io.linlan.doc.common.util.UrlUtil.urlJoin(path, queryParamsMap);
            url = StringUtilsExt.removeQuotes(url);
            url = apiMethodDoc.getServerUrl() + "/" + url;
            url = io.linlan.doc.common.util.UrlUtil.simplifyUrl(url);
            CurlRequest curlRequest = CurlRequest.builder()
    ...
    private void mappingParamProcess(String str, Map<String, String> pathParamsMap) {
        String param = StringUtilsExt.removeQuotes(str);
        String paramName;
        String paramValue;
        if (param.contains("=")) {
    ...
    private static void setTornaArrayTags(List<JavaParameter> javaParameters, ApiMethodDoc apiMethodDoc, JavaClass returnClass) {
        ...
        //request tags
        if (io.linlan.doc.common.util.CollectionUtil.isNotEmpty(javaParameters)) {
