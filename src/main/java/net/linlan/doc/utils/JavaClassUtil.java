...
import net.linlan.doc.constants.DocValidatorAnnotationEnum;
import net.linlan.doc.constants.ValidatorAnnotations;
import net.linlan.doc.model.DocJavaField;
        ...
        if (null == cls1) {
            ...
            if (cls1.isInterface() &&
                ...
                for (JavaMethod javaMethod : methods) {
                    ...
                    if (methodName.startsWith("get") && !"get".equals(methodName) && paramSize == 0) {
                        methodName = StringUtilsExt.firstToLowerCase(methodName.substring(3));
                        enable = true;
                    } else if (methodName.startsWith("is") && !"is".equals(methodName) && paramSize == 0) {
                        methodName = StringUtilsExt.firstToLowerCase(methodName.substring(2));
                        enable = true;
                    }
            ...
            for (JavaField javaField : cls1.getFields()) {
                ...
                if (fieldName.startsWith("is") && ("boolean".equals(subTypeName))) {
                    fieldName = StringUtilsExt.firstToLowerCase(fieldName.substring(2));
                }
    ...
    public static String getClassTagsValue(final JavaClass cls, final String tagName, boolean checkComments) {
        if (StringUtils.isNotEmpty(tagName)) {
            ...
            for (int i = 0; i < tags.size(); i++) {
                ...
                if (StringUtils.isEmpty(value) && checkComments) {
